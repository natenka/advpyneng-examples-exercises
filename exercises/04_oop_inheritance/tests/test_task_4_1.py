...
def test_send_config_commands(first_router_from_devices_yaml):
    ...
    assert type(cfg_output) == str, "Метод send_config_commands должен возвращать строку"
    assert 'interface loopback123' in cfg_output and 'ip address 123.1.2.3 255.255.255.255' in cfg_output, "В выводе должны быть строки с командами: 'interface loopback123', 'ip address 123.1.2.3 255.255.255.255'"
    r1.close()
...
def test_send_show_command(first_router_from_devices_yaml):
    ...
    assert 'Loopback123' in show_output and '123.1.2.3' in show_output, "В выводе sh ip int br должен быть интерфейс Loopback123 и IP-адрес 123.1.2.3"
    r1.close()
