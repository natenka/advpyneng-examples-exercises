...
def test_get_ip_from_cfg_is_generator():
    return_value = task_8_1.get_ip_from_cfg('config_r1.txt')
...
def test_get_ip_from_cfg_yield_value():
    ...
    assert ('10.1.1.1', '255.255.255.255') in all_results, "Функция вернула неправильный результат"
...
def test_get_ip_from_cfg_new_file(tmpdir):
    ...
    dest_filename = tmpdir.mkdir("test_tasks").join("task_8_1.txt")
    dest_filename.write(config)
    # проверяем результат
    return_value = task_8_1.get_ip_from_cfg(dest_filename)
