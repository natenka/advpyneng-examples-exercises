...
def test_func_created():
    ...
    check_function_exists(task_8_4a, "retry")


def test_retry_success(capsys):
    @task_8_4a.retry(times=3, delay=1)
    def do_thing(a, b):
        print("done")
        return a + b
...
def test_retry_failed(capsys):
    @task_8_4a.retry(times=2, delay=1)
    def do_thing(a, b):
        print("done")
        return None
    ...
    assert (
        out.count("Повторное подключение") == 2
    ), "При повторном выполнении функции на stdout должно выводиться сообщение"
