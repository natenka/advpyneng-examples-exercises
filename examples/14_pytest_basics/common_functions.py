...
def check_function_exists(module, function_name):
    assert hasattr(module, function_name) and inspect.isfunction(getattr(module, function_name)), f"Надо создать функцию с именем {function_name}"


def check_function_params(function, param_count, param_names=None):
    arg_info = inspect.getfullargspec(function)
    assert len(arg_info.args) == param_count, f"У функции {function.__name__} должно быть {param_count} параметров"
    if param_names:
        assert set(arg_info.args) == set(param_names), f"У функции должны быть такие параметры: {','.join(param_names)}"
